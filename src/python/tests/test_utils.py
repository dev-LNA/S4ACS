...
def test_parameters() -> None:
    ...
    assert cfg_file.image_path == Path(Path().resolve().anchor) / "images" / "today"
    assert (
        cfg_file.log_file_path
        == Path(Path().resolve().anchor) / "sparc4" / "log" / "s4acs1"
    )
    assert cfg_file.log_level == 10
