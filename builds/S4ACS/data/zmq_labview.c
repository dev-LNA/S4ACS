...
// to simplify error handling, make sure we return 0 on success and -ve on error
#ifdef _WIN32
...
{
	int ret, i; void *sock;
	/* validate -- the socket may already have been closed */
	CHECK_SOCK( sockobj );
	sock = sockobj->sock;
	...
	/* remove from context */
	if ( sockobj->ctx ) {
		/* NB: the context may already have been destroyed */
		ctx_obj *ctxobj = sockobj->ctx;
		i = bonzai_clip( ctxobj->socks, sockobj );
...
EXPORT int lvzmq_ctx_destroy( ctx_obj **pinstdata, ctx_obj *ctxobj, int flags )
{
	...
	int i;
	/* validate -- the context may already have been destroyed */
	CHECK_CTX( ctxobj );
	ctx = ctxobj->ctx;
	tree = ctxobj->socks;
	DEBUGMSG( "TERM on %p (%p) with %u sockets", ctxobj->ctx, ctxobj, tree->n );
	...
	if ( flags ) {
		...
		/* it is important that none of the sockets are undergoing blocking operations!! */
		for ( i = 0; i < tree->n; ++i )
		{
			/* note we MUST NOT close blocking sockets; as they are in use in another thread and must be released there */
			if ((( sockobj = tree->elem[i] ) != NULL ) && !( sockobj->flags & FLAG_BLOCKING ))
			{
		...
		/* if we were called from lvzmq_ctx_destroy_abort() then the zmq_ctx_destroy() call has hanged so don't continue */
		if ( pinstdata && ( *pinstdata == ctxobj )) return 0;
	}
	/* remove from list of valid objects to prevent issues from attempting to terminate the same context multiple times */
	bonzai_clip( validobj, ctxobj );
	/* close the context -- this will hang until the termination is complete! */
	if ( pinstdata ) *pinstdata = ctxobj;
	zmq_ctx_destroy( ctx );
	DEBUGMSG( "  CTX destroy done" );
	/* any unclosed sockets must be detached from this (now dead) context */
	for ( i = 0; i < tree->n; ++i )
		if (( sockobj = tree->elem[i] ) != NULL )
			sockobj->ctx = NULL;
	if ( pinstdata ) *pinstdata = NULL;
	/* destroy the book-heeping objects */
...
EXPORT int lvzmq_ctx_destroy_abort( ctx_obj **pinstdata )
{
	...
	if ( pinstdata && *pinstdata ) {
		...
		/* close ALL associated sockets to unblock zmq_ctx_destroy() */
		lvzmq_ctx_destroy( pinstdata, *pinstdata, FLAG_INTERRUPT );
	}
	return 0;
}
...
EXPORT int lvzmq_ctx_create_unreserve( bonzai** pinstdata )
{
	...
	bonzai_sort( insttree );
	DEBUGMSG( "UNRESERVE instance %p -> %i contexts", insttree, insttree->n );
	for ( i = 0; i < insttree->n; ++i )
		if (( ctxobj = insttree->elem[i] ) != NULL ) {
			/* eliminate this context */
			lvzmq_ctx_destroy( NULL, ctxobj, FLAG_INTERRUPT );
		}
...
EXPORT int lvzmq_recv_abort( sock_obj** pinstdata )
{
	...
	*/
	...
	lvzmq_ctx_destroy( NULL, sockobj->ctx, FLAG_INTERRUPT ); // <!------ we rely on the function unblocking and succeeding
	*pinstdata = NULL;
...
{
	int ret = 0, flags = 0, n = 0;
	bonzai *list;
	CRITCHECK;
	DEBUGMSG( "RECV_MULTI on %p into %p (%u)", sockobj->sock, h, DSGetHandleSize(h) );
	// clear input handle
	DSSetHSzClr( h, 4 );
	if ( pinstdata ) *pinstdata = NULL;	// not used within lvzmq_receiver_thread()
	list = bonzai_init( NULL );
	do {
		// get the next message part
		UHandle ptr = DSNewHClr( 4 );
		if ( !ptr ) {
			// shit, out of memory
			ret = -ENOBUFS;
			break;
		...
		if ( ret < 0 ) {
			// throw away the temporary pointer and stop the loop
			DSDisposeHandle( ptr );
			break;			
		} else {
			// add it to the stack
	...
	n = list->n;
	DEBUGMSG( "RECV_MULTI on %p got %d", sockobj->sock, n );
	DSSetHandleSize( h, 8+n*sizeof( void* ));
	memcpy( LVALIGN( *h+4 ), list->elem, n*sizeof( void* ));
	*( u32* )*h = n;
	// LabVIEW now "owns" the string pointers and must free them
	bonzai_free( list );
	return ret;
}
...
EXPORT int lvzmq_recv_multi_timeout( sock_obj **pinstdata, sock_obj *sockobj, char** h, long timeout )
{
	...
	CHECK_SOCK( sockobj );
	// TODO: better handle the case where the interrupt occurs during the poll call - the usage of pinstdata is different
	*pinstdata = NULL;
...
EXPORT int lvzmq_send( sock_obj *sockobj, const UHandle h, int *flags, const char *group )
{
	int ret = 0;
	uint32_t l = 0;
	zmq_msg_t msg;
...
EXPORT int lvzmq_send_multi( sock_obj *sockobj, char** h )
{
	...
	for ( n = *( u32* )*h; n > 0; ++ptr, --n )
	{
		...
		ret = lvzmq_send( sockobj, ( UHandle )*ptr, &flags, NULL );
		if ( ret ) break;
	}
	return ret;
}
...
EXPORT int lvzmq_get_monitor_event( sock_obj** pinstdata, sock_obj *s, int* intval, UHandle strval )
{
	...
	if ( ret < 0 ) {
		DSDisposeHandle( buffer );	/* prevent memory leak */
		return ret;
	}
...
EXPORT int lvzmq_proxy( ctx_obj **pinstdata, sock_obj *frontend, sock_obj *backend, sock_obj *capture, sock_obj *control )
{
	...
	*pinstdata = frontend->ctx;
	...
	ret = zmq_proxy_steerable( frontend->sock, backend->sock, capture ? capture->sock : NULL, control ? control->sock : NULL );
...
EXPORT int lvzmq_proxy_abort( ctx_obj **pinstdata )
{
	...
	if ( pinstdata && *pinstdata )
		lvzmq_ctx_destroy( NULL, *pinstdata, FLAG_INTERRUPT );
	return 0;
}
...
{
#ifdef DEBUG
	bonzai_sort( validobj );	// remove NULLs
	DEBUGMSG( "DETACH library, %u objects", validobj->n );
	for ( int i = 0; i < validobj->n; ++i )
		DEBUGMSG( "    %i: %p", i, validobj->elem[i] );
#endif
...
EXPORT int lvzmq_connect( sock_obj *s, const char *addr )
{
	int ret;
	CHECK_SOCK(s);
	DEBUGMSG( "CONNECTing %p to %s", s->sock, addr );
...
EXPORT int lvzmq_join( sock_obj *s, const char *group )
{
	int ret;
	CHECK_SOCK(s);
...
EXPORT int lvzmq_leave( sock_obj *s, const char *group )
{
	int ret;
	CHECK_SOCK(s);
...
EXPORT int lvzmq_has(const char *capability)
{
	return zmq_has(capability);
}

EXPORT int lvzmq_curve_public(char *public_out, char *secret_in)
{
	// outputs a public key for the corresponding given secret key, both 41 octets of Z85
	return RET0(zmq_curve_public(public_out, secret_in));
}

EXPORT int lvzmq_curve_keypair(char *public_out, char *secret_out)
{
	// generate new public and secret keys, output arrays must be at least 41 octets large
	return RET0(zmq_curve_keypair(public_out, secret_out));
}
